...
| 字段 | 是否必需 | 约束 |
|-------|----------|-------------|
| `name` | Yes | 最多 64 个字符。仅可包含小写字母、数字和连字符。不得以下划线或连字符开头或结尾。 |
| `description` | Yes | 最多 1024 个字符。不能为空。描述该技能做什么以及何时使用。 |
| `license` | No | 许可证名称或对捆绑许可证文件的引用。 |
| `compatibility` | No | 最多 500 个字符。指示环境要求（预期产品、系统包、网络访问等）。 |
| `metadata` | No | 任意键值映射，用于额外元数据。 |
| `allowed-tools` | No | 以空格分隔的预先批准工具字符串，技能可使用它们。（实验性） |
...
2. **指令**（建议 < 5000 个 token）：`SKILL.md` 正文在技能激活时加载
